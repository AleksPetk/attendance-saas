"""Owner Apple OAuth business logic (login, register, link)."""

from __future__ import annotations

import logging
from dataclasses import dataclass

from django.contrib.auth import get_user_model
from django.db import IntegrityError, transaction
from django.http import HttpResponseRedirect
from django.utils import timezone

from accounts.apple_oauth_client import (
    AppleOAuthClientError,
    exchange_authorization_code,
    verify_apple_id_token,
)
from accounts.apple_oauth_settings import (
    apple_oauth_account_security_result_url,
    apple_oauth_frontend_result_url,
    apple_oauth_redirect_uri,
)
from accounts.apple_oauth_state import (
    INTENT_LINK,
    INTENT_LOGIN,
    INTENT_REGISTER,
    INTENT_VERIFY,
    AppleOAuthStateError,
    consume_apple_oauth_state,
)
from accounts.email_uniqueness import email_address_claimed, normalize_owner_email
from accounts.owner_auth_provider_models import OwnerAuthProvider, OwnerAuthProviderLink
from accounts.owner_authentication import complete_owner_authentication
from accounts.owner_sensitive_auth import record_owner_oauth_reauth
from accounts.services import provision_verified_owner

logger = logging.getLogger("accounts.apple_oauth")
User = get_user_model()


class AppleOAuthResultCode:
    SUCCESS = "success"
    TWO_FACTOR_REQUIRED = "two_factor_required"
    LINKED = "linked"
    ALREADY_LINKED = "already_linked"
    EXISTING_ACCOUNT_CONNECT_REQUIRED = "existing_account_connect_required"
    NO_ACCOUNT = "no_account"
    APPLE_ALREADY_LINKED = "apple_already_linked"
    DIFFERENT_APPLE_LINKED = "different_apple_linked"
    LEGAL_ACKNOWLEDGEMENT_REQUIRED = "legal_acknowledgement_required"
    OAUTH_NOT_CONFIGURED = "oauth_not_configured"
    INVALID_STATE = "invalid_state"
    AUTHENTICATION_FAILED = "authentication_failed"
    EMAIL_NOT_VERIFIED = "email_not_verified"
    EMAIL_MISSING = "email_missing"
    AUTHENTICATION_REQUIRED = "authentication_required"
    INVALID_INTENT = "invalid_intent"
    VERIFIED = "verified"


@dataclass(frozen=True)
class AppleIdentity:
    subject: str
    email: str
    email_verified: bool


def redirect_apple_oauth_result(result_code: str) -> HttpResponseRedirect:
    return HttpResponseRedirect(apple_oauth_frontend_result_url(result_code))


def redirect_apple_account_security_result(result_code: str) -> HttpResponseRedirect:
    return HttpResponseRedirect(apple_oauth_account_security_result_url(result_code))


def _apple_email_verified(claims: dict, email: str) -> bool:
    raw = claims.get("email_verified")
    if raw in (True, "true", "True"):
        return True
    if email:
        # Sign in with Apple only shares verified email addresses.
        return True
    return False


def parse_apple_identity(claims: dict) -> AppleIdentity:
    subject = str(claims.get("sub") or "").strip()
    email = normalize_owner_email(claims.get("email") or "")
    email_verified = _apple_email_verified(claims, email)
    return AppleIdentity(subject=subject, email=email, email_verified=email_verified)


def get_apple_provider_link(*, subject: str) -> OwnerAuthProviderLink | None:
    return OwnerAuthProviderLink.objects.filter(
        provider=OwnerAuthProvider.APPLE,
        provider_subject=subject,
    ).select_related("user").first()


def get_owner_apple_link(user) -> OwnerAuthProviderLink | None:
    return OwnerAuthProviderLink.objects.filter(
        user=user,
        provider=OwnerAuthProvider.APPLE,
    ).first()


def update_apple_provider_link_snapshot(
    link: OwnerAuthProviderLink,
    identity: AppleIdentity,
) -> None:
    """
    Update informational provider fields without changing User.email.

    Apple may omit email on later authorizations; preserve the prior snapshot.
    """
    update_fields = ["last_used_at"]
    link.last_used_at = timezone.now()
    if identity.email:
        link.provider_email = identity.email
        link.provider_email_verified = identity.email_verified
        update_fields.extend(["provider_email", "provider_email_verified"])
    link.save(update_fields=update_fields)


def create_apple_provider_link(user, identity: AppleIdentity) -> OwnerAuthProviderLink:
    return OwnerAuthProviderLink.objects.create(
        user=user,
        provider=OwnerAuthProvider.APPLE,
        provider_subject=identity.subject,
        provider_email=identity.email or "",
        provider_email_verified=identity.email_verified,
        last_used_at=timezone.now(),
    )


def _finalize_owner_login(request, user) -> HttpResponseRedirect:
    response = complete_owner_authentication(request, user)
    if response.status_code == 200:
        return redirect_apple_oauth_result(AppleOAuthResultCode.SUCCESS)
    if response.status_code == 403:
        code = response.data.get("code")
        if code == "two_factor_required":
            return redirect_apple_oauth_result(AppleOAuthResultCode.TWO_FACTOR_REQUIRED)
        if code == "email_not_verified":
            return redirect_apple_oauth_result(AppleOAuthResultCode.EMAIL_NOT_VERIFIED)
    if response.status_code == 404:
        return redirect_apple_oauth_result(AppleOAuthResultCode.AUTHENTICATION_FAILED)
    return redirect_apple_oauth_result(AppleOAuthResultCode.AUTHENTICATION_FAILED)


def _validate_apple_identity_for_registration(identity: AppleIdentity) -> str | None:
    if not identity.email:
        return AppleOAuthResultCode.EMAIL_MISSING
    if not identity.email_verified:
        return AppleOAuthResultCode.EMAIL_NOT_VERIFIED
    return None


def _login_existing_apple_link(request, link: OwnerAuthProviderLink, identity: AppleIdentity):
    update_apple_provider_link_snapshot(link, identity)
    return _finalize_owner_login(request, link.user)


@transaction.atomic
def _register_new_apple_owner(identity: AppleIdentity):
    user = User.objects.create_user(
        email=identity.email,
        password=None,
        email_verified=True,
    )
    user.set_unusable_password()
    user.save(update_fields=["password"])
    create_apple_provider_link(user, identity)
    provision_verified_owner(user)
    return user


def handle_apple_oauth_login(request, identity: AppleIdentity) -> HttpResponseRedirect:
    link = get_apple_provider_link(subject=identity.subject)
    if link is not None:
        return _login_existing_apple_link(request, link, identity)

    if identity.email and email_address_claimed(identity.email):
        return redirect_apple_oauth_result(
            AppleOAuthResultCode.EXISTING_ACCOUNT_CONNECT_REQUIRED
        )
    return redirect_apple_oauth_result(AppleOAuthResultCode.NO_ACCOUNT)


def handle_apple_oauth_register(
    request,
    identity: AppleIdentity,
    *,
    legal_acknowledgement: bool,
) -> HttpResponseRedirect:
    if not legal_acknowledgement:
        return redirect_apple_oauth_result(
            AppleOAuthResultCode.LEGAL_ACKNOWLEDGEMENT_REQUIRED
        )

    validation_error = _validate_apple_identity_for_registration(identity)
    if validation_error is not None:
        return redirect_apple_oauth_result(validation_error)

    if get_apple_provider_link(subject=identity.subject) is not None:
        return redirect_apple_oauth_result(AppleOAuthResultCode.APPLE_ALREADY_LINKED)

    if email_address_claimed(identity.email):
        return redirect_apple_oauth_result(
            AppleOAuthResultCode.EXISTING_ACCOUNT_CONNECT_REQUIRED
        )

    try:
        user = _register_new_apple_owner(identity)
    except IntegrityError:
        logger.warning("Apple registration race for subject=%s", identity.subject)
        link = get_apple_provider_link(subject=identity.subject)
        if link is not None:
            return _login_existing_apple_link(request, link, identity)
        if email_address_claimed(identity.email):
            return redirect_apple_oauth_result(
                AppleOAuthResultCode.EXISTING_ACCOUNT_CONNECT_REQUIRED
            )
        return redirect_apple_oauth_result(AppleOAuthResultCode.AUTHENTICATION_FAILED)

    return _finalize_owner_login(request, user)


def handle_apple_oauth_link(
    request,
    identity: AppleIdentity,
    *,
    owner_user_id: int,
) -> HttpResponseRedirect:
    actor = request.user
    if not getattr(actor, "is_authenticated", False) or actor.pk != owner_user_id:
        return redirect_apple_account_security_result(AppleOAuthResultCode.AUTHENTICATION_REQUIRED)

    if getattr(actor, "is_staff", False) or getattr(actor, "is_superuser", False):
        return redirect_apple_account_security_result(AppleOAuthResultCode.AUTHENTICATION_FAILED)

    existing_for_subject = get_apple_provider_link(subject=identity.subject)
    if existing_for_subject is not None and existing_for_subject.user_id != actor.pk:
        return redirect_apple_account_security_result(AppleOAuthResultCode.APPLE_ALREADY_LINKED)

    owner_link = get_owner_apple_link(actor)
    if owner_link is not None:
        if owner_link.provider_subject == identity.subject:
            update_apple_provider_link_snapshot(owner_link, identity)
            return redirect_apple_account_security_result(AppleOAuthResultCode.ALREADY_LINKED)
        return redirect_apple_account_security_result(AppleOAuthResultCode.DIFFERENT_APPLE_LINKED)

    try:
        create_apple_provider_link(actor, identity)
    except IntegrityError:
        existing_for_subject = get_apple_provider_link(subject=identity.subject)
        if existing_for_subject is not None and existing_for_subject.user_id == actor.pk:
            update_apple_provider_link_snapshot(existing_for_subject, identity)
            return redirect_apple_account_security_result(AppleOAuthResultCode.ALREADY_LINKED)
        return redirect_apple_account_security_result(AppleOAuthResultCode.APPLE_ALREADY_LINKED)

    return redirect_apple_account_security_result(AppleOAuthResultCode.LINKED)


def handle_apple_oauth_verify(
    request,
    identity: AppleIdentity,
    *,
    owner_user_id: int,
) -> HttpResponseRedirect:
    actor = request.user
    if not getattr(actor, "is_authenticated", False) or actor.pk != owner_user_id:
        return redirect_apple_account_security_result(AppleOAuthResultCode.AUTHENTICATION_REQUIRED)

    if getattr(actor, "is_staff", False) or getattr(actor, "is_superuser", False):
        return redirect_apple_account_security_result(AppleOAuthResultCode.AUTHENTICATION_FAILED)

    owner_link = get_owner_apple_link(actor)
    if owner_link is None or owner_link.provider_subject != identity.subject:
        return redirect_apple_account_security_result(AppleOAuthResultCode.AUTHENTICATION_FAILED)

    update_apple_provider_link_snapshot(owner_link, identity)
    record_owner_oauth_reauth(request, actor, OwnerAuthProvider.APPLE)
    return redirect_apple_account_security_result(AppleOAuthResultCode.VERIFIED)


def process_apple_oauth_callback(request, *, code: str | None, state: str | None) -> HttpResponseRedirect:
    if not code or not state:
        return redirect_apple_oauth_result(AppleOAuthResultCode.AUTHENTICATION_FAILED)

    try:
        pending = consume_apple_oauth_state(request, state)
    except AppleOAuthStateError:
        return redirect_apple_oauth_result(AppleOAuthResultCode.INVALID_STATE)

    redirect_uri = apple_oauth_redirect_uri(request)
    try:
        token_data = exchange_authorization_code(redirect_uri=redirect_uri, code=code)
        claims = verify_apple_id_token(
            token_data["id_token"],
            expected_nonce=pending.nonce,
        )
    except AppleOAuthClientError:
        return redirect_apple_oauth_result(AppleOAuthResultCode.AUTHENTICATION_FAILED)

    identity = parse_apple_identity(claims)

    if pending.intent == INTENT_LOGIN:
        return handle_apple_oauth_login(request, identity)
    if pending.intent == INTENT_REGISTER:
        return handle_apple_oauth_register(
            request,
            identity,
            legal_acknowledgement=pending.legal_acknowledgement,
        )
    if pending.intent == INTENT_LINK:
        if pending.owner_user_id is None:
            return redirect_apple_account_security_result(AppleOAuthResultCode.INVALID_STATE)
        return handle_apple_oauth_link(
            request,
            identity,
            owner_user_id=int(pending.owner_user_id),
        )
    if pending.intent == INTENT_VERIFY:
        if pending.owner_user_id is None:
            return redirect_apple_account_security_result(AppleOAuthResultCode.INVALID_STATE)
        return handle_apple_oauth_verify(
            request,
            identity,
            owner_user_id=int(pending.owner_user_id),
        )
    return redirect_apple_oauth_result(AppleOAuthResultCode.INVALID_INTENT)

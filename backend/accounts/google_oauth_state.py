"""Session-backed OAuth state and nonce for owner Google sign-in."""
...
from accounts.google_oauth_settings import google_oauth_state_ttl_seconds

OWNER_GOOGLE_OAUTH_SESSION_KEY = "_owner_google_oauth_pending"
...
def create_google_oauth_state(
    ...
    *,
    ...
    pending = GoogleOAuthPendingState(
...
def load_google_oauth_state(request) -> GoogleOAuthPendingState | None:
    raw = request.session.get(OWNER_GOOGLE_OAUTH_SESSION_KEY)
    if not isinstance(raw, dict):
        return None
    try:
        return GoogleOAuthPendingState(
            state=str(raw["state"]),
            nonce=str(raw["nonce"]),
...
def clear_google_oauth_state(request) -> None:
    request.session.pop(OWNER_GOOGLE_OAUTH_SESSION_KEY, None)
    request.session.modified = True


def consume_google_oauth_state(request, submitted_state: str) -> GoogleOAuthPendingState:
    pending = load_google_oauth_state(request)
    if pending is None:
        raise GoogleOAuthStateError("invalid_state")

    created_at = _parse_timestamp(pending.created_at)
    if created_at is None:
        clear_google_oauth_state(request)
        raise GoogleOAuthStateError("invalid_state")

    age = timezone.now() - created_at
    if age > timedelta(seconds=google_oauth_state_ttl_seconds()):
        clear_google_oauth_state(request)
        raise GoogleOAuthStateError("expired_state")

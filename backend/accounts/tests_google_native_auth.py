...
class GoogleNativeCompleteEndpointTests(TestCase):
    ...
    def test_malformed_request_rejected(self):
        ...
        response = self._post({"identity_token": "tok", "intent": "bogus"})
        self.assertEqual(response.status_code, 400)
...
@override_settings(**NATIVE_TEST_SETTINGS)
class GoogleNativeLinkEndpointTests(TestCase):
    def setUp(self):
        self.owner, self.org = create_owner(email="link-native@example.com")
        self.owner.set_unusable_password()
        self.owner.save(update_fields=["password"])
        OwnerAuthProviderLink.objects.create(
            user=self.owner,
            provider=OwnerAuthProvider.APPLE,
            provider_subject="apple-already-on-owner",
            provider_email="link-native@example.com",
            provider_email_verified=True,
        )
        self.client = Client()
        self.client.force_login(self.owner)
    ...
    def _claims(self, *, sub="google-link-sub", email="google-link@example.com"):
        return {
            "sub": sub,
            "email": email,
            "email_verified": True,
    ...
    def _post_link(self, *, claims):
        with patch(
            ...
            return self.client.post(
                "/api/auth/google/native/",
                data={"identity_token": "fake-token", "intent": "link"},
                content_type="application/json",
            )

    def test_authenticated_owner_links_google(self):
        response = self._post_link(claims=self._claims())
        self.assertEqual(response.status_code, 200, response.content)
        self.assertEqual(response.json()["code"], GoogleOAuthResultCode.LINKED)
        self.assertEqual(self.client.session.session_key, self.session_key)
        link = OwnerAuthProviderLink.objects.get(
            user=self.owner,
            provider=OwnerAuthProvider.GOOGLE,
        )
        self.assertEqual(link.provider_subject, "google-link-sub")
        self.assertTrue(
            OwnerAuthProviderLink.objects.filter(
                user=self.owner,
                provider=OwnerAuthProvider.APPLE,
    ...
    def test_already_linked_to_current_owner_is_idempotent(self):
        OwnerAuthProviderLink.objects.create(
            user=self.owner,
            provider=OwnerAuthProvider.GOOGLE,
            provider_subject="google-link-sub",
            provider_email="google-link@example.com",
            provider_email_verified=True,
        )
        response = self._post_link(claims=self._claims())
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["code"], GoogleOAuthResultCode.ALREADY_LINKED)
        self.assertEqual(
            OwnerAuthProviderLink.objects.filter(
                provider=OwnerAuthProvider.GOOGLE,
                provider_subject="google-link-sub",
    ...
    def test_provider_linked_to_another_owner_rejected(self):
        other, _org = create_owner(email="other-google@example.com")
        OwnerAuthProviderLink.objects.create(
            user=other,
            provider=OwnerAuthProvider.GOOGLE,
            provider_subject="google-taken-sub",
        )
        response = self._post_link(claims=self._claims(sub="google-taken-sub"))
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json()["code"], GoogleOAuthResultCode.GOOGLE_ALREADY_LINKED)
        self.assertFalse(
            OwnerAuthProviderLink.objects.filter(
    ...
    def test_email_match_alone_does_not_merge_accounts(self):
        create_owner(email="same-email@example.com")
        response = self._post_link(
            claims=self._claims(sub="google-new-sub", email="same-email@example.com")
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["code"], GoogleOAuthResultCode.LINKED)
        self.assertEqual(
            OwnerAuthProviderLink.objects.get(
                provider=OwnerAuthProvider.GOOGLE,
                provider_subject="google-new-sub",
            ).user_id,
            self.owner.pk,
        )

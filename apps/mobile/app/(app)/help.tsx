...
function ContactRow({ title, description, onPress }: { title: string; description: string; onPress: () => void }) {
  return (
    <Pressable accessibilityRole="button" onPress={onPress} style={({ pressed }) => [styles.row, pressed && styles.pressed]} testID="help-contact-entry">
      <View style={styles.icon}><Ionicons color={colors.blue} name="mail-outline" size={21} /></View>
...
export default function HelpScreen() {
  ...
  const helpGroupIndex = useMemo(
    () => groups.findIndex((group) => group.rows.some((row) => row.slug === "support" || row.slug === "faq") || group.id === "help"),
    [groups],
  );
  ...
  return <Screen style={styles.screen}><ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void load(true)} tintColor={colors.blue} />}>
    ...
    {groups.map((group, index) => (
      <SectionCard key={group.id} title={group.label}>
        {group.rows.map((document) => (
          <DocumentRow document={document} key={document.id} onPress={() => openDocument(document.slug)} />
        ))}
        {index === helpGroupIndex ? (
          <ContactRow
    ...
    {!loading && helpGroupIndex < 0 ? (
      <SectionCard title={t("help.resources")}>
        <ContactRow
          description={t("help.contact.description")}
          title={t("help.contact.title")}
          onPress={openContact}

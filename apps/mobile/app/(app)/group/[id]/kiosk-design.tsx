...
import { Alert as NativeAlert, Image, Pressable, ScrollView, StyleSheet, Switch, Text, useWindowDimensions, View } from "react-native";
...
type MediaState = { headerLogo: MediaSelection | null; footerLogo: MediaSelection | null; background: MediaSelection | null; removeHeaderLogo: boolean; removeFooterLogo: boolean; removeBackground: boolean };
...
export default function KioskDesignScreen() {
  ...
  const mediaDirty = Boolean(media.headerLogo || media.footerLogo || media.background || media.removeHeaderLogo || media.removeFooterLogo || media.removeBackground);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [designData, presetData, group, settings] = await Promise.all([
        ...
        api.get<{ group_type?: string }>(endpoints.group(id)),
  ...
  const patch = useCallback((path: string, value: unknown) => {
    setHistory((current) => {
      ...
      const next = patchKioskDesignConfig(base, path, value);
  ...
  async function save() {
    ...
    try {
      ...
      appendMedia(body, "background", media.background, "remove_background", media.removeBackground);
      const result = await api.request<KioskDesignData>(endpoints.kioskDesign(id), { method: "PUT", formData: body });
      const normalizedDocument = normalizeKioskDesignDocument(result);
      const saved = normalizedDocument.config;
      setDesign(normalizedDocument);
      setHistory([saved]); setHistoryIndex(0); setMedia(EMPTY_MEDIA);
  ...
  const sections: Array<{ value: EditorSection; label: string }> = [
    ...
    { value: mode === "card" ? "cards" : "input", label: t(mode === "card" ? "kiosk.cards" : "kiosk.input") },
  ];
  ...
  return (
    <Screen style={styles.screen}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Alert message={error} />
        <View style={[styles.builderBody, width >= 768 && styles.builderBodyTablet]}>
        <View style={styles.previewColumn}><LivePreview config={config} design={design} media={media} mode={mode} sampleCount={sampleCount} t={t} /></View>
          ...
          <SectionCard title={t("kiosk.header")} description={t("kiosk.headerHint")}>
            <View style={styles.editorStack}>
              ...
              {header.enabled ? <>
                ...
                <MediaPicker label={t("kiosk.logo")} current={Boolean(media.headerLogo || (!media.removeHeaderLogo && design?.header_logo_url))} onPick={(value) => setMedia((current) => ({ ...current, headerLogo: value, removeHeaderLogo: false }))} onRemove={() => setMedia((current) => ({ ...current, headerLogo: null, removeHeaderLogo: true }))} t={t} />
              </> : null}
            </View>
          </SectionCard>
        ) : null}
        ...
        {activeSection === "main" ? (
          <SectionCard title={t("kiosk.main")} description={t("kiosk.mainHint")}>
            <View style={styles.editorStack}>
              <BackgroundEditor background={main.background} imageAvailable={Boolean(media.background || (!media.removeBackground && design?.main_background_image_url))} label={t("kiosk.background")} onChange={(value) => patch("main.background", value)} t={t} />
              <MediaPicker label={t("kiosk.backgroundImage")} current={Boolean(media.background || (!media.removeBackground && design?.main_background_image_url))} onPick={(value) => { setMedia((current) => ({ ...current, background: value, removeBackground: false })); patch("main.background.mode", "image"); }} onRemove={() => { setMedia((current) => ({ ...current, background: null, removeBackground: true })); patch("main.background.mode", "solid"); }} t={t} />
          ...
          <SectionCard title={t("kiosk.footer")} description={t("kiosk.footerHint")}>
            <View style={styles.editorStack}>
              ...
              {footer.enabled ? <>
                ...
                <MediaPicker label={t("kiosk.footerImage")} current={Boolean(media.footerLogo || (!media.removeFooterLogo && design?.footer_logo_url))} onPick={(value) => setMedia((current) => ({ ...current, footerLogo: value, removeFooterLogo: false }))} onRemove={() => setMedia((current) => ({ ...current, footerLogo: null, removeFooterLogo: true }))} t={t} />
              </> : null}
            </View>
          </SectionCard>
        ) : null}
        ...
        {activeSection === "cards" ? (
          <SectionCard title={t("kiosk.cards")} description={t("kiosk.cardsHint")}>
            <View style={styles.editorStack}>
              ...
              <OptionPicker label={t("kiosk.testParticipants")} value={String(sampleCount)} options={[6, 12, 20, 50, 100].map((count) => ({ value: String(count), label: String(count) }))} onChange={(value) => setSampleCount(Number(value))} />
        ...
        {activeSection === "input" ? (
          <SectionCard title={t("kiosk.input")} description={t("kiosk.cardsHint")}>
            <View style={styles.editorStack}>
        ...
        <View style={styles.saveActions}>
          <View style={styles.historyActions}><Button label={t("kiosk.undo")} variant="secondary" disabled={historyIndex === 0} onPress={() => setHistoryIndex((value) => Math.max(0, value - 1))} /><Button label={t("kiosk.redo")} variant="secondary" disabled={historyIndex >= history.length - 1} onPress={() => setHistoryIndex((value) => Math.min(history.length - 1, value + 1))} /></View>
...
function MediaPicker({ label, current, onPick, onRemove, t }: { label: string; current: boolean; onPick: (value: MediaSelection) => void; onRemove: () => void; t: (key: string) => string }) {
  ...
  return <View style={styles.controlGroup}><Text style={styles.controlLabel}>{label}</Text><Button label={current ? t("kiosk.replaceImage") : t("kiosk.uploadImage")} loading={busy} variant="secondary" onPress={() => void pick()} />{current ? <Button label={t("kiosk.removeImage")} variant="danger" onPress={onRemove} /> : null}</View>;
}
...
function BackgroundSurface({ background, imageUrl, localImageUri, style, children }: { background: KioskVisualDesign["main"]["background"]; imageUrl?: string | null; localImageUri?: string | null; style?: object; children: ReactNode }) {
  if (background.mode === "image" && (localImageUri || imageUrl)) return <View style={[style, { backgroundColor: background.color, overflow: "hidden" }]}>{localImageUri ? <Image resizeMode="cover" source={{ uri: localImageUri }} style={StyleSheet.absoluteFill} /> : <AuthenticatedImage resizeMode="cover" style={StyleSheet.absoluteFill} url={imageUrl || ""} />}{children}</View>;
...
function LivePreview({ config, design, media, mode, sampleCount, t }: { config: KioskDesignConfig; design: KioskDesignData | null; media: MediaState; mode: "card" | "input"; sampleCount: number; t: (key: string) => string }) {
  ...
  const people = Array.from({ length: sampleCount }, (_, index) => `CS-${String(index + 1).padStart(4, "0")}`);
  const headerLogo = media.removeHeaderLogo ? "" : media.headerLogo?.uri || design?.header_logo_url || "";
  const footerLogo = media.removeFooterLogo ? "" : media.footerLogo?.uri || design?.footer_logo_url || "";
  return <View style={styles.previewShell}><View style={styles.previewHeading}><Text style={styles.previewHeadingText}>{t("kiosk.livePreview")}</Text><Text style={styles.previewUnsaved}>{t("kiosk.previewUpdatesLive")}</Text></View><View style={styles.previewCanvas}>{config.header.enabled ? <BackgroundSurface background={config.header.background} style={styles.previewHeader}>{headerLogo ? (media.headerLogo ? <Image source={{ uri: headerLogo }} style={styles.previewLogo} /> : <AuthenticatedImage style={styles.previewLogo} url={headerLogo} />) : null}<Text numberOfLines={1} style={[styles.previewHeaderText, { color: config.header.title.color, textAlign: config.header.alignment as "left" | "center" | "right" }]}>{config.header.title.text || t("kiosk.previewHeader")}</Text></BackgroundSurface> : null}<BackgroundSurface background={config.main.background} imageUrl={media.removeBackground ? "" : design?.main_background_image_url} localImageUri={media.background?.uri} style={styles.previewMain}><Text numberOfLines={1} style={[styles.previewMainTitle, { color: config.main.title.color, textAlign: config.main.title.alignment as "left" | "center" | "right" }]}>{config.main.title.text || t("kiosk.previewWelcome")}</Text>{mode === "card" ? <ScrollView contentContainerStyle={styles.previewPeople}>{people.map((code) => <View key={code} style={[styles.previewCard, cardStyle]}><View style={styles.previewAvatar} /><View style={styles.previewPersonCopy}><Text numberOfLines={1} style={styles.previewPersonName}>{t("kiosk.previewPerson")}</Text><Text style={styles.previewPersonCode}>{code}</Text></View></View>)}</ScrollView> : <View style={styles.previewInput}><View style={styles.previewInputField}><Text style={styles.previewPersonCode}>{t("kiosk.participantCode")}</Text></View><View style={[styles.previewButton, buttonStyle]}><Text style={styles.previewButtonText}>{t("auth.continue")}</Text></View></View>}</BackgroundSurface>{config.footer.enabled ? <BackgroundSurface background={config.footer.background} style={styles.previewFooter}>{footerLogo ? (media.footerLogo ? <Image source={{ uri: footerLogo }} style={styles.previewLogo} /> : <AuthenticatedImage style={styles.previewLogo} url={footerLogo} />) : null}<Text numberOfLines={1} style={[styles.previewFooterText, { color: config.footer.text.color, textAlign: config.footer.text.alignment as "left" | "center" | "right" }]}>{config.footer.text.lines[0] || t("kiosk.previewFooter")}</Text></BackgroundSurface> : null}</View></View>;
}
...
const styles = StyleSheet.create({
  ...
  builderBody: { gap: space.lg },
  builderBodyTablet: { flexDirection: "row", alignItems: "flex-start" },
  previewColumn: { flex: 1, minWidth: 0 },
  editorColumn: { flex: 1, minWidth: 0, gap: space.lg },
  ...
  stepper: { flexDirection: "row", alignItems: "center", gap: space.sm }, stepperButton: { width: 38, height: 38, alignItems: "center", justifyContent: "center", borderRadius: radii.sm, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surfaceMuted }, stepperButtonText: { fontSize: 22, lineHeight: 24, color: colors.bluePressed }, stepperValue: { ...type.captionStrong, color: colors.text, minWidth: 42, textAlign: "center" }, saveActions: { gap: space.sm }, historyActions: { flexDirection: "row", gap: space.sm },
  previewShell: { borderRadius: radii.lg, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, overflow: "hidden", ...shadows.md }, previewHeading: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: space.md, paddingVertical: space.sm, borderBottomWidth: 1, borderBottomColor: colors.border }, previewHeadingText: { ...type.captionStrong, color: colors.text }, previewUnsaved: { fontSize: 11, color: colors.textMuted }, previewCanvas: { height: 420, backgroundColor: colors.surfaceMuted }, previewHeader: { minHeight: 42, flexDirection: "row", alignItems: "center", gap: space.sm, justifyContent: "center", paddingHorizontal: space.md }, previewLogo: { width: 34, height: 24, resizeMode: "contain" }, previewHeaderText: { ...type.captionStrong, flex: 1 }, previewMain: { flex: 1, alignItems: "center", justifyContent: "center", gap: space.sm, padding: space.md }, previewMainTitle: { ...type.bodyStrong, width: "100%" }, previewPeople: { width: "100%", alignItems: "center", gap: space.xs, paddingBottom: space.sm }, previewCard: { width: "82%", minHeight: 50, flexDirection: "row", alignItems: "center", gap: space.sm, borderRadius: 10, padding: space.sm, backgroundColor: colors.surface }, previewCardElevated: { ...shadows.md }, previewCardFlat: { backgroundColor: colors.surfaceMuted }, previewCardBordered: { borderWidth: 2, borderColor: colors.blue }, previewAvatar: { width: 32, height: 32, borderRadius: 18, backgroundColor: colors.blueSoft }, previewPersonCopy: { flex: 1 }, previewPersonName: { ...type.captionStrong, color: colors.text }, previewPersonCode: { fontSize: 11, color: colors.textMuted }, previewInput: { width: "82%", gap: space.sm }, previewInputField: { minHeight: 42, justifyContent: "center", borderRadius: 8, borderWidth: 1, borderColor: colors.borderStrong, backgroundColor: colors.surface, paddingHorizontal: space.md }, previewButton: { minHeight: 32, justifyContent: "center", alignItems: "center", paddingHorizontal: space.lg, backgroundColor: colors.blue }, previewButtonRounded: { borderRadius: 8 }, previewButtonPill: { borderRadius: 999 }, previewButtonFlat: { borderRadius: 2 }, previewButtonText: { fontSize: 11, fontWeight: "700", color: colors.surface }, previewFooter: { minHeight: 34, flexDirection: "row", alignItems: "center", gap: space.sm, justifyContent: "center", paddingHorizontal: space.md }, previewFooterText: { fontSize: 11, flex: 1 },
});

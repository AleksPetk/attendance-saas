...
export default function HomeScreen() {
  const { api, authState, locale, t, refreshWorkspace, revision } = useApp();
  ...
  const load = useCallback(async (refresh = false) => { refresh ? setRefreshing(true) : setLoading(true); setError(""); try { setData(await api.get<Dashboard>(endpoints.dashboard())); } catch (caught) { setError(caught instanceof ApiError ? caught.message : t("common.error")); } finally { setLoading(false); setRefreshing(false); } }, [api, revision, t]);
  ...
  return (
    <Screen style={styles.screen}>
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void refresh()} tintColor={colors.blue} />}>
        ...
        <View style={[styles.dashboardPanels, tablet && styles.dashboardPanelsTablet]}>
          <View style={styles.recentPanel}><SectionCard title={t("dashboard.recentActivity")} description={t("dashboard.recentDescription")}>
            {data?.recent_activity?.length ? <View>{data.recent_activity.map((item, index) => <Pressable key={item.id} onPress={() => router.push("/(app)/(tabs)/history")} style={[styles.activityRow, index > 0 && styles.rowBorder]}><ActionIcon action={item.action} /><View style={styles.activityMain}><View style={styles.activityTitle}><Text numberOfLines={1} style={styles.activityName}>{item.person?.name || t("common.unknown")}</Text><ActivityBadge action={item.action} label={dashboardActionLabel(item.action, t)} /></View><Text numberOfLines={1} style={styles.activityMeta}>{item.group_name || ""}{item.source ? ` · ${item.source}` : ""}</Text></View><View style={styles.activityTime}><Text style={styles.activityWhen}>{item.performed_at ? formatDateTime(item.performed_at, locale) : ""}</Text><Ionicons color={colors.textMuted} name="chevron-forward" size={17} /></View></Pressable>)}</View> : <EmptyPanel body={t("dashboard.emptyBody")} icon="pulse-outline" title={t("dashboard.emptyTitle")} action={<Button label={t("dashboard.goGroups")} onPress={() => router.push("/(app)/(tabs)/groups")} variant="secondary" />} />}
          </SectionCard></View>
          <View style={styles.quickPanel}><SectionCard title={t("dashboard.quickActions")}>
            <View style={styles.actions}>
              {showMembers ? <QuickAction icon="person-add-outline" label={t("members.add")} onPress={() => router.push("/(app)/member/new")} /> : null}
              <QuickAction icon={canConfigure ? "add-circle-outline" : "layers-outline"} label={canConfigure ? t("groups.add") : t("dashboard.openGroups")} onPress={() => router.push(canConfigure ? "/(app)/group/new" : "/(app)/(tabs)/groups")} />
...
function dashboardActionLabel(action: string | undefined, t: (key: string) => string) {
  ...
  if (action === "break_end") return t("kiosk.breakEnd");
  return action?.replace(/_/g, " ") || t("history.action");
}

function ActivityBadge({ action, label }: { action?: string; label: string }) {
  const tone = action === "check_in" ? styles.badgeCheckIn : action === "check_out" ? styles.badgeCheckOut : styles.badgeBreak;
  const textTone = action === "check_in" ? styles.badgeTextCheckIn : action === "check_out" ? styles.badgeTextCheckOut : styles.badgeTextBreak;
  return <View style={[styles.activityBadge, tone]}><Text style={[styles.activityBadgeText, textTone]}>{label}</Text></View>;
}
...
const styles = StyleSheet.create({ screen: { padding: 0 }, content: { padding: space.lg, paddingTop: topComfortGap, paddingBottom: space.xxxl, gap: space.lg, width: "100%", maxWidth: 1120, alignSelf: "center" }, stats: { flexDirection: "row", flexWrap: "wrap", gap: space.md }, dashboardPanels: { gap: space.lg }, dashboardPanelsTablet: { flexDirection: "row", alignItems: "flex-start" }, recentPanel: { flex: 2, minWidth: 0 }, quickPanel: { flex: 1, minWidth: 0 }, activityRow: { minHeight: 66, flexDirection: "row", alignItems: "center", gap: space.md, paddingVertical: space.md }, rowBorder: { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: colors.border }, activityMain: { flex: 1, minWidth: 0 }, activityTitle: { flexDirection: "row", alignItems: "center", flexWrap: "wrap", gap: space.sm }, activityName: { ...type.bodyStrong, color: colors.text, flexShrink: 1 }, activityBadge: { borderRadius: radii.pill, paddingHorizontal: space.sm, paddingVertical: 2 }, activityBadgeText: { ...type.captionStrong }, badgeCheckIn: { backgroundColor: colors.successSoft }, badgeTextCheckIn: { color: colors.successText }, badgeCheckOut: { backgroundColor: colors.blueSoft }, badgeTextCheckOut: { color: colors.bluePressed }, badgeBreak: { backgroundColor: colors.cyanSoft }, badgeTextBreak: { color: colors.textSecondary }, activityMeta: { ...type.caption, color: colors.textMuted }, activityTime: { maxWidth: 115, flexDirection: "row", alignItems: "center", gap: 2 }, activityWhen: { ...type.caption, color: colors.textSecondary, textAlign: "right", flexShrink: 1 }, actions: { gap: space.xs }, quickAction: { minHeight: 52, flexDirection: "row", alignItems: "center", gap: space.md, borderRadius: radii.sm, paddingHorizontal: space.sm }, quickIcon: { width: 36, height: 36, borderRadius: 10, backgroundColor: colors.primarySoft, alignItems: "center", justifyContent: "center" }, quickLabel: { ...type.bodyStrong, color: colors.text, flex: 1 }, pressed: { backgroundColor: colors.surfaceMuted } });

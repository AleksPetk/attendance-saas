...
export default function MembersScreen() {
  ...
  const load = useCallback(async (refresh = false, term = search, nextStatus = status) => { const request = ++generation.current; refresh ? setRefreshing(true) : setLoading(true); setError(""); const params = new URLSearchParams({ status: nextStatus }); if (term.trim()) params.set("search", term.trim()); try { const next = await api.get<Member[]>(`${endpoints.members()}?${params.toString()}`); if (request === generation.current) setRows(next); } catch (caught) { if (request === generation.current) setError(caught instanceof ApiError ? caught.message : t("common.error")); } finally { if (request === generation.current) { setLoading(false); setRefreshing(false); } } }, [api, search, status, t]);

...
import { SecurityActions } from "../../src/components/SecurityActions";
...
export default function SecurityScreen() {
  const { api, authState, t } = useApp(); const [revision, setRevision] = useState(0); const [account, setAccount] = useState<Account | null>(null); const [loading, setLoading] = useState(true); const [error, setError] = useState("");
  useEffect(() => { let cancelled = false; void api.get<Account>(endpoints.account()).then((value) => { if (!cancelled) setAccount(value); }).catch((caught) => { if (!cancelled) setError(caught instanceof Error ? caught.message : t("common.error")); }).finally(() => { if (!cancelled) setLoading(false); }); return () => { cancelled = true; }; }, [api, t, revision]);

...
export function FilterTabs<T extends string>({ value, options, onChange, selected = "plain" }: { value: T; options: ReadonlyArray<{ value: T; label: string }>; onChange: (value: T) => void; selected?: "plain" | "brand" }) {
  return <View style={styles.filterTabs}>{options.map((option) => { const active = option.value === value; const brand = selected === "brand"; return <Pressable accessibilityRole="tab" accessibilityState={{ selected: active }} key={option.value} onPress={() => onChange(option.value)} style={({ pressed }) => [styles.filterTab, active && (brand ? styles.filterTabBrand : styles.filterTabActive), pressed && brand && styles.filterTabPressed]}>{active && brand ? <LinearGradient colors={[colors.blue, colors.cyan]} end={{ x: 1, y: 1 }} start={{ x: 0, y: 0 }} style={styles.filterTabFill} /> : null}<Text numberOfLines={1} style={[styles.filterTabText, brand && styles.filterTabTextNeutral, active && (brand ? styles.filterTabTextBrand : styles.filterTabTextActive)]}>{option.label}</Text></Pressable>; })}</View>;
}
...
const styles = StyleSheet.create({
  ...
  filterTabs: { flexDirection: "row", padding: 3, borderRadius: radii.sm, backgroundColor: colors.surfaceSubtle }, filterTab: { flex: 1, minWidth: 0, minHeight: 38, alignItems: "center", justifyContent: "center", borderRadius: 6, paddingHorizontal: space.sm }, filterTabActive: { backgroundColor: colors.surface, ...shadows.sm }, filterTabBrand: { shadowColor: colors.blue, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.18, shadowRadius: 7, elevation: 2 }, filterTabFill: { position: "absolute", top: 0, right: 0, bottom: 0, left: 0, borderRadius: 6 }, filterTabPressed: { opacity: 0.86 }, filterTabText: { ...type.captionStrong, color: colors.textMuted, textAlign: "center" }, filterTabTextNeutral: { color: colors.textSecondary }, filterTabTextActive: { color: colors.blue }, filterTabTextBrand: { color: colors.surface },

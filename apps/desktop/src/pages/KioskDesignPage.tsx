...
export function KioskDesignPage() {
  ...
  useEffect(() => { const refresh = () => { if (!dirty) void load(); }; window.addEventListener("focus", refresh); return () => window.removeEventListener("focus", refresh); }, [dirty, load]);
  ...
  return <div className="desktop-kiosk-builder">
    <div className="desktop-kiosk-builder-canvas"><EditorPreview snapshot={snapshot} response={response} mode={mode} display={display} settings={settings} sampleCount={sampleCount} /></div>
...
function EditorPreview({ snapshot, response, mode, display, settings, sampleCount }: { snapshot: Snapshot; response: DesignResponse | null; mode: Mode; display: { showName: boolean; showCode: boolean; showEmail: boolean }; settings: KioskSettingsApi | null; sampleCount: number }) {
  const { t } = useApp();
...
function HeaderEditor({ snapshot, commit, catalog, response }: EditorProps) { const { t } = useApp(); const { header } = snapshot.config; return <EditorSection><Toggle label={t("kiosk.enabled")} value={header.enabled} onChange={(enabled) => commit((next) => { next.config.header.enabled = enabled; })} />{header.enabled ? <><BackgroundEditor value={header.background} modes={["solid", "gradient"]} onChange={(background) => commit((next) => { next.config.header.background = background; })} /><Alignment value={header.alignment} onChange={(alignment) => commit((next) => { next.config.header.alignment = alignment; })} /><EditorGroup title={t("kiosk.headerTitle")}><input maxLength={150} placeholder={t("kiosk.optionalTitle")} value={header.title.text || ""} onChange={(event) => commit((next) => { next.config.header.title.text = event.target.value; })} /></EditorGroup><TextEditor value={header.title} catalog={catalog} onChange={(value) => commit((next) => { next.config.header.title = { ...next.config.header.title, ...value }; })} /><MediaEditor kind="header" current={Boolean(snapshot.media.headerLogo || (!snapshot.media.removeHeaderLogo && response?.header_logo_url))} file={snapshot.media.headerLogo} onPick={(file) => commit((next) => { next.media.headerLogo = file; next.media.removeHeaderLogo = false; next.config.header.logo ||= { size: .75 }; })} onRemove={() => commit((next) => { next.media.headerLogo = null; next.media.removeHeaderLogo = true; next.config.header.logo = null; })} />{header.logo ? <Range label={t("kiosk.logoSize")} value={Number(header.logo.size) || .75} min={.35} max={1} step={.05} format={(value) => `${Math.round(value * 100)}%`} onChange={(size) => commit((next) => { next.config.header.logo = { ...(next.config.header.logo || {}), size }; })} /> : null}</> : null}</EditorSection>; }

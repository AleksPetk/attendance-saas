...
function ParticipantCreate({ group, groupId, onClose, onSaved }: { group: Group; groupId: string; onClose: () => void; onSaved: () => void }) {
  const { api, t } = useApp(); const [mode, setMode] = useState<"member" | "visitor">("member"); const [available, setAvailable] = useState<AvailableGroupMember[]>([]); const [availableLoading, setAvailableLoading] = useState(true); const [memberId, setMemberId] = useState(""); const [memberEmails, setMemberEmails] = useState<string[]>([""]); const [memberPin, setMemberPin] = useState(""); const [visitorName, setVisitorName] = useState(""); const [visitorEmails, setVisitorEmails] = useState<string[]>([""]); const [visitorPin, setVisitorPin] = useState(""); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [fields, setFields] = useState<Record<string, string>>({});
  const classScoped = groupId.includes("/classes/");
  useEffect(() => { let active = true; setAvailableLoading(true); void api.get<AvailableGroupMember[]>(endpoints.groupAvailableMembers(groupId)).then((value) => { if (active) setAvailable(value); }).catch((caught) => { if (active) setError(formatError(caught, t("common.error"))); }).finally(() => { if (active) setAvailableLoading(false); }); return () => { active = false; }; }, [api, groupId, t]);
